...
export const scoreEffectsSchema = z.strictObject({
  archetypes: weightMap.default({}),
  motivations: weightMap.default({}),
  decision_styles: weightMap.default({}),
  shadow: weightMap.default({}),
  traits: weightMap.default({}),
});
...
export const archetypeSchema = z
  .strictObject({
    id: z.enum(ARCHETYPE_KEYS),
    slug: z.enum(ARCHETYPE_KEYS),
    name: z.string().min(2),
    short_description: z.string().min(10),
    core_desire: z.string().min(5),
    core_fear: z.string().min(5),
    core_motivation: z.string().min(5),
    strengths: phrases,
    shadow_traits: phrases,
    growth_direction: phrases,
    decision_patterns: phrases,
    relationship_patterns: phrases,
    leadership_patterns: phrases,
...
export const choiceSchema = z.strictObject({
  ...
  female_variant: z.string().min(10).nullable().default(null),
  scores: scoreEffectsSchema,
});
export const sceneSchema = z.strictObject({
  id: z.string().regex(/^[a-z][a-z0-9_-]{2,63}$/),
  act: z.number().int().min(1).max(24),
  order: z.number().int().min(1),
  title: z.string().min(3),
  narrative: z.string().min(50).max(4000),
  ...
  is_pressure: z.boolean().default(false),
...
export const journeyContentSchema = z.strictObject({
  ...
  title: z.string().min(3),
  journey_version: z.string().regex(/^\d+\.\d+(?:\.\d+)?$/),
  scoring_version: z.string().regex(/^\d+\.\d+(?:\.\d+)?$/),
  archetypes: z.array(archetypeSchema).length(12),
  dimensions: z.array(dimensionSchema).min(30),
  scenes: z.array(sceneSchema).min(3).max(48),
});
